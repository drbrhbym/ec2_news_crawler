...
def getNewsView(urlQueue):
    while True:
        ...
        if responseCode == 200:
            ...
            news_html = BeautifulSoup(news_response, features="html5lib") # features="html5lib" for Windows
            ...
            news_tag = news.find("div", class_="ndArticle_moreNewlist").find("h2").text.replace("《", "").replace("》",
                                                                                                                 "")
            # 將新聞觀看數放入佇列
            viewQueue.put({"id": "apple-" + tag_dict[news_tag] + "-" + news_url.split("/")[-2],
                           "news_link": news_url,
                           "view": news_view,
...
if __name__ == "__main__":
    ...
    for date in date_list:
        ...
        ## 如果檔案存在
        if os.path.exists(date + "_apple_news_view.json"):
            # 開啟之前紀錄新聞觀看數的檔案
            with open(date + "_apple_news_view.json", "r", encoding="utf-8") as f:
                file_content = json.load(f)
            # 將依照爬取日期分類的新聞觀看數存檔
            with open(date + "_apple_news_view.json", "w", encoding="utf-8") as f:
                # 將每筆新的新聞觀看數加入之前的紀錄
